...
# Multithreaded - runs game#call in Ractor
...
module Whack
  class Runners::Ruby2dParallel
    ...
    def run
      ...
      Thread.new do
        loop do
          ...
          earliest_update = earliest_update_time_from(loop_start)

          @clock = loop_start - @start

          game_ractor.send(env)
          game_result = game_ractor.take

          @state_mutex.synchronize do
            # mutex prevents render loop from reading state
            # as it's being written here
            @game_state, @game_objects = game_result
          end

          record_game_time(mono_time - loop_start)

          wait_until_next_update(loop_start)
    ...
    def stop
      # TODO: stop @thread/ractor somehow (some kind of poison?)
    end
    ...
    def game_ractor
      @game_ractor ||= Ractor.new(@game) do |game|
        loop do
          env = Ractor.receive
          Ractor.yield(game.call(env))
        end
      end
    end
    ...
    def render_debug_text
      ...
      # TODO: calculate internally, don't rely on game
      ups = (1.0 / (average_update_time || 1)).floor(1)

      Text.new(
        "UPS #{ups.round(1)}/#{MAX_UPS}",
      ...
      wait_ms = update_ms - game_ms

      Text.new(
        "UGW #{update_ms}ms #{game_ms}ms #{wait_ms}ms",
